...
It is best understood as a post-failure constrained repair mode.
...
Use this skill when the user wants an explicit loop or harness that keeps iterating against a failing test until it passes or a stop condition is hit.

Use `patch-first-debugging` when the user wants a one-off surgical debugging pass without a repeated automated loop.

In short:

- `patch-first-debugging` = minimal patch mindset
- `test-fix-loop` = minimal patch mindset plus repeated test-driven repair loop
...
Set concrete loop limits up front.

Defaults:

- max attempts per failing test: 3
- unchanged-error threshold: 2 consecutive identical failures
- max touched files per patch: 3
- max diff hunks per patch: 5
- stop immediately if the failure becomes ambiguous

If the user gives stricter limits, use those instead.
...
When multiple tests fail, do not try to solve the whole suite at once.

Pick one failing test using this order:

1. the first newly failing test in the current run
2. the most specific failing test with a direct traceback into app code
3. the failure with the smallest obvious blast radius

If the failures appear unrelated, stop and say the loop needs a narrower target.
...
## Loop runner contract

If you are helping build or operate the loop, keep the machine-facing contract strict.

Input to each repair attempt:
...
- or the exact token `CANNOT_FIX_CONFIDENTLY`

Optional runner metadata:

- touched files count
- diff hunk count
- whether the error changed after the patch
...
Also reject or stop when:

- the candidate patch exceeds configured file or hunk limits
- the selected failing test is no longer the active target
- the patch appears to fix one failure by broad unrelated edits
...
- legacy or messy code with a known failing test
- regression repair where the desired outcome is already concrete
- low-ambiguity bug fixing where speed matters more than redesign

This skill is a poor fit for:

- feature work
- broad cleanup
- missing tests
- failures that need product or architecture decisions first
